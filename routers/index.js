...
router.get('/', async (req, res) => {
	...
	var bv = await BaiViet.find({ KiemDuyet: 1 }) // Chỉ lấy bài viết đã được duyệt
		.sort({ NgayDang: -1 })
		.populate('ChuDe')
		.populate('TaiKhoan')
		.limit(12).exec();
	var xnn = await BaiViet.find({ KiemDuyet: 1 }) // Chỉ lấy bài viết đã được duyệt
		.sort({ LuotXem: -1 }) // sort theo lượt xem giảm dần (nhiều nhất trước)
		.populate('ChuDe')
		.populate('TaiKhoan')
		.limit(3).exec();
